...
def test_selector_test_coverage_integrity():
    ...
    for loader, module_name, is_pkg in pkgutil.walk_packages(factorflow.__path__, factorflow.__name__ + "."):
        try:
            module = importlib.import_module(module_name)
            for name, obj in inspect.getmembers(module, inspect.isclass):
                # 条件：是 Selector 的子类，不是 Selector 本身，且定义在 factorflow 包内
                if issubclass(obj, Selector) and obj is not Selector and obj.__module__.startswith("factorflow"):
