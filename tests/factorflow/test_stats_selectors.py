...
from factorflow.stats_selectors.select_kbest_na_safe import SelectKBestAnova, SelectKBestChi2, SelectKBestMutualInfo
from factorflow.stats_selectors.select_na_ratio import SelectNARatio
from factorflow.stats_selectors.select_percentile_na_safe import SelectPercentileAnova
...
def test_select_fpr(sample_data):
    ...
    X_cat = pd.DataFrame({"cat_1": [0, 0, 1, 1, 2], "cat_2": [0, 1, 0, 1, 0]})
...
def test_select_percentile(sample_data):
    ...
    X_distinct = pd.DataFrame(
        {"feat_high": [1, 1, 5, 5, 10, 10], "feat_mid": [1, 2, 1, 2, 1, 2], "feat_low": [1, 1, 1, 1, 1, 1]}
    )
